...
def calculate_rssi_and_distance(df: pd.DataFrame, position: list[float, float, float]) -> pd.DataFrame:
    ...
    df['Distance_Meter'] = df['Distance'] / 100  # Convert to meters
...
def calculate_log_distance_parameters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Fit model
    popt, _ = curve_fit(log_distance_model, df['Distance_Meter'], df['RSSI'])
...
def discretize_by_delta(df: pd.DataFrame, dt: int = 0) -> pd.DataFrame:
    ...
    discretized_df["RSSI_Std"] = df.groupby(["Time_Bucket"])["RSSI"].std().reset_index(drop=True).fillna(0)
...
def calculate_rssi_estimated_distance(df: pd.DataFrame, rssi_0: float, n: float) -> pd.DataFrame:
    ...
    '''
    df['RSSI_Distance_Meter'] = 10 ** ((rssi_0 - df['RSSI']) / (10 * n))
    df['RSSI_Distance'] = df['RSSI_Distance_Meter'] * 100  # Convert to cm
