...
def aoa_local_mobile_kf(df: pd.DataFrame, anchor_ids: list[int], config: dict, delta: int, threshold: int):
    ...
    # vs.visualize_distance_error_with_heatmap_timebucket(all_results['ekf'], 'X_Real', 'Y_Real', 'X_EKF', 'Y_EKF', vmin=0, vmax=200, title="Extended Local")

    # vs.visualize_distance_error_with_heatmap_timebucket(all_results['ukf-pre'], 'X_Real', 'Y_Real', 'X_UKF', 'Y_UKF', vmin=0, vmax=200, title="Unscented Local")
    vs.visualize_distance_error_with_heatmap_timebucket(all_results['ukf'], 'X_Real', 'Y_Real', 'X_UKF', 'Y_UKF', vmin=0, vmax=200, title="Unscented Local")
    vs.visualize_distance_error_with_heatmap_timebucket(all_results['wls'], 'X_Real', 'Y_Real', 'X_WLS', 'Y_WLS', vmin=0, vmax=200, title="Weighted Local")
...
def mobility():
    ...
    gt_path = os.path.join(base_dir, f"../dataset/0421/gt/set-1.csv")
    gt_df = pd.read_csv(gt_path)

    ms_path_1 = os.path.join(base_dir, f"../dataset/0421/beacons/anchor1/diagonal-set.csv")
    ms_df_1 = pd.read_csv(ms_path_1)
    ms_path_2 = os.path.join(base_dir, f"../dataset/0421/beacons/anchor2/diagonal-set.csv")
    ms_df_2 = pd.read_csv(ms_path_2)
    ms_path_3 = os.path.join(base_dir, f"../dataset/0421/beacons/anchor3/diagonal-set.csv")
    ms_df_3 = pd.read_csv(ms_path_3)
    ms_path_4 = os.path.join(base_dir, f"../dataset/0421/beacons/anchor4/diagonal-set.csv")
    ms_df_4 = pd.read_csv(ms_path_4)
    # gt_path = os.path.join(base_dir, f"../dataset/0416/gt/rectangular-mid-2.csv")
    # gt_df = pd.read_csv(gt_path)
    ...
    anchors_df = anchors_df.reset_index()
    # filtered_df = anchors_df[(anchors_df["Time_Bucket"] > 53) & (anchors_df["Time_Bucket"] < 69)]

    # filtered_df.to_csv("filtered_data-53-69.csv", index=False)
